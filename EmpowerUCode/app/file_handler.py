...
#         # Handle capturing the correct answer
#         if line.startswith("###ANSWER"):
#             print(f"Debug: Found '###ANSWER' at line {line_num}")
#             capture_answer = True  # Start capturing answer lines
#             multiline_answer = []  # Reset answer buffer
#             continue
#
...
#                 capture_answer = False
#                 current_question["Answer"] = "\n".join(multiline_answer).strip()
#                 print(f"Debug: Captured multi-line answer at line {line_num}: {current_question['Answer']}")
#             elif line == '"""':
#                 continue
#             else:
#                 multiline_answer.append(line)
#                 print(f"Debug: Appending to multiline answer at line {line_num}: {line}")
#                 continue
#
#         # Start a new question
#         if line.startswith("###QUESTION"):
#             if current_question:  # Save the previous question
#                 quiz_data.append(current_question)
#                 print(f"Debug: Saved question at line {line_num}: {current_question}")
#             current_question = {"Q": lines[line_num].strip(), "Hints": [], "Options": []}  # Ensure 'Options' is always initialized
#             print(f"Debug: New question started at line {line_num}: {current_question['Q']}")
#
#         # Parse the question type
#         elif line.startswith("###TYPE"):
#             current_question["Type"] = lines[line_num].strip()
#             print(f"Debug: Question type found at line {line_num}: {current_question['Type']}")
#
#         # Capture options for MCQ or Drag-and-Drop questions
#         elif line.startswith("###OPTIONS"):
#             current_question["Options"] = []
...
#             current_question["Options"].append(line)
#             print(f"Debug: Captured option at line {line_num}: {line}")
#
#         # Capture hints for progressive hinting
#         elif line.startswith("###HINT"):
#             hints = [hint.strip() for hint in lines[line_num].split(",")]
#             current_question["Hints"] = hints
#             print(f"Debug: Captured hints at line {line_num}: {current_question['Hints']}")
#
#     # Add the last question if present
#     if current_question:
#         quiz_data.append(current_question)
#         print(f"Debug: Saved last question: {current_question}")
#
#     print(f"Debug: Completed parsing. Total questions parsed: {len(quiz_data)}")
#     return quiz_data
