...
class InfoSecurityContent(tk.Frame):
    def __init__(self, master, master_previous, topic_index):
        ...
        with open(f"./data/Information_Security/information_security_topic_{self.topic_index}.txt", "r") as rf:
            rf = rf.readlines()
        ...
        self.backbutton = BackButton(self.master, self, "Back to Information Security HomePage")
        self.backbutton.back_button.config(command=self.hide_content)
        
    def show_content(self):
        self.place(relx=0, rely=.01, anchor=tk.NW)
        self.backbutton.place(relx=.12,rely=.95,anchor=tk.CENTER)
